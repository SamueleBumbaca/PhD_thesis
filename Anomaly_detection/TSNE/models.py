...
class DINOv2Base(nn.Module):
    def __init__(self, model_name):
        ...
        try:
            ...
            # Get the correct hub directory for the current platform
            import os
            hub_dir = torch.hub.get_dir()
            cache_path = os.path.join(hub_dir, "checkpoints", f"{model_name}.pth")
            print(f"Loading DINOv2 from cache: {cache_path}")
...
# Define the model dictionaries at module level so they can be imported
...
# Combine dictionaries for easier access
...
# Function to get all available model names
