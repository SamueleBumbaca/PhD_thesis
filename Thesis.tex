...
\abstract{\textbf{Background and Research Gap:} Agricultural research for Plant Protection Product (PPP) development relies on hypothesis statistical testing. The European and Mediterranean Plant Protection Organizzation (EPPO) set the standards to perform tests in all its member countries. EPPO recocnized traditional statistical approaches address environmental variability through experimental design features like randomized controls. These methods require experimentalists to identify environmental variability before data collection. This a priori identification of variance sources depends heavily on experimentalist expertise rather than systematic statistical procedures, creating a significant limitation in agricultural field studies. Geostatistical methods offer an alternative approach by enabling post-hoc estimation of spatial environmental variability through variograms or spline fitting techniques. These methods are also recognized by EPPO, but they requires spatially referenced data collection.

\textbf{Research Objectives:} This research investigates the applicability of geomatics technologies for generating spatially referenced datasets across all EPPO-recognized observation categories (binary, nominal, ordinal, discrete, and continuous), with the goal of demonstrating how these techniques can facilitate the adoption of geostatistical methods in hypothesis testing for agricultural research within the EPPO standard framework.

\textbf{Methodology:} Three study cases were conducted addressing different variable types: (1) continuous variables through machine learning-based maize seedling counting using photogrammetry and computer vision; (2) ordinal variables through automated phytotoxicity scoring using multispectral imaging and custom feature extraction; and (3) binary/nominal variables through anomaly detection for plant health classification using pre-trained models and unsupervised learning.

\textbf{Key Results:} For continuous variables, transformer-mixed architectures (RT-DETR) required fewer training samples (approximately 60) compared to pure CNN models (110-130 samples) to achieve EPPO benchmark performance ($R^2$ > 0.85), though in-domain training data proved essential regardless of architecture. For ordinal variables, machine learning effectively reproduced phytotoxicity scoring ($\kappa$ > 0.7) through photogrammetric 3D models and spectral imaging, enabling conversion from ordinal to continuous scales for enhanced statistical analysis. For binary/nominal variables, pre-trained models successfully performed plant health classification (accuracy > 0.85) using anomaly detection without task-specific training.
